...
for (i in 0:46) {
  ...
  test_x <- select(test_data, -target)[, "stock_id"]
...
for (i in 0:46) {
  ...
  # Sélection des données d'entraînement et de test
  train_data <- filter(data_n, date_id > train_start_day & date_id <= train_start_day + 10)[, c(most_important_features,"target")]
  test_data <- filter(data_n, date_id > test_start_day & date_id <= test_start_day + 10)[, c(most_important_features,"target")]
  ...
  formula_string <- as.formula(paste("target ~", paste("s(", most_important_features, ")", collapse = " + ")))
